package com.hjw.excel.excle.utils;
...
public class ExcelUtil {
    ...
    /**
     * 在readExcel方法外再做一层封装，将生产的model类转换成pojo实体类
     * @param excel 文件
     * @param rowModel Model类映射，继承 BaseRowModel 类
     * @param pojo 实体类，对应数据库
     * @return Excel 数据 list
     */
    public static <T1 extends BaseRowModel,T2> List<T2> readExcel(MultipartFile excel, Class<T1> rowModel, Class<T2> pojo) throws ExcelException{
        List<T1> list= readExcel(excel, rowModel);
        return MyBeanCopy.convert(list, pojo);
    }
    ...
    public static <T extends BaseRowModel> void writeExcel(HttpServletResponse response, List<T> list,
        ...
        if(excelTypeEnum == ExcelTypeEnum.XLSX) {
            ExcelWriter writer = EasyExcelFactory.getWriterWithTempAndHandler(null,
                    getOutputStream(fileName, response, excelTypeEnum), excelTypeEnum, true, new MyWriterHandler<>(classType));
